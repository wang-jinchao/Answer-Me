...
# -*- coding: utf-8 -*-
"""
每日一走 —— liteapp 步数写入（集成版，供 account_runner 调用）
...
BASE_URL = os.environ.get("WALK_BASE_URL") or "<WALK_BASE_URL>"
...
def _mask_name(name):
    """对绑定人真实姓名脱敏，避免日志/报告明文暴露敏感身份关联（openid↔姓名）。

    保留首尾、中间打码；单字则原字+***；空值返回 '?'。
    """
    s = (name or "").strip()
    if not s:
        return "?"
...
def run_walk(account: dict) -> dict:
    ...
    **不需要 decrypt、不依赖 enc/iv/key**（decrypt 仅回读 30 天历史，维护态不阻断写入）。
    ...
    logger.info("每日一走 账号 %s 绑定校验完成 userStep=%s", account.get("name") or account.get("username") or "?", idx.get("userStep"))

    if target_name and target_name not in str(bound):
        result["status"] = "failed"
        result["reason"] = (f"绑定人不是目标【{target_name}】（当前绑定={_mask_name(bound)}），"
                            f"已停止，未做任何写入；请先在微信小程序登录目标账号切回绑定")
        logger.warning("每日一走 绑定漂移：期望 %s 实际 %s，中止防误写", target_name, _mask_name(bound))
        return result
