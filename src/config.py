...
def get_urls():
    """返回答题主页 URL 列表（如 rsbindex0823 / rsbindex0820）。

    解析与兼容：
      1. TARGET_URL 支持逗号分隔多 URL（"url1,url2"）；
      2. 也支持 TARGET_URL_2 / TARGET_URL_3 / ... 逐号扩展；
      3. 若都没有，回退 HOMEPAGE_URL / HOMEPAGE（单值）；
      4. 都没有则报错。
    去重保序，同一 URL 不会重复跑到。
    注意：每日一炼（AI运动会）走独立 ai-sports 路径，不在此列表内。
    """
    urls = []
    seen = set()

    raw = os.environ.get("TARGET_URL") or ""
    ...
    i = 2
    while True:
        v = os.environ.get(f"TARGET_URL_{i}")
        if not v:
            break
        v = v.strip()
    ...
    if not urls:
        fb = os.environ.get("HOMEPAGE_URL") or os.environ.get("HOMEPAGE")
        if fb and fb.strip():
            return [fb.strip()]

    if not urls:
        raise RuntimeError(
            "未提供答题主页。请在 Secrets 中配置 TARGET_URL（逗号分隔多 URL 或 TARGET_URL_2/3/...）"
            "或 HOMEPAGE_URL。"
        )
    return urls
