...
自动完成目标学习平台的每日任务（每日一学 / 每日一看 / 每日一练 / 每日一答，**以及可选的每日一走步数同步**）并汇总积分报告。
...
当前实现为**串行顺序执行**：`main.py` 用 `for acc in accounts: run_account(acc, url)`，
每个账号独立启动浏览器、完成后关闭，**任意时刻只有一个账号在跑**（等价于早先设计里的 `MAX_WORKERS=1`）。

> 说明：旧文档曾描述 `daily.yml` 的 `MAX_WORKERS` 环境变量控制并发数，但当前代码并未实现该变量，
> 实际始终串行——此处以代码为准。串行既能最大程度降低同 IP 风控风险，也避免多浏览器抢占 Actions runner 资源。
